...
import { checkIfContentAlreadyPostedForArticle, loadPostedArticles, savePostedArticle, savePostedArticleContent, log } from './functions/utils';
...
async function runBot() {
	try {
		...
		log(LogLevel.DEBUG, 'Posted articles loaded:', postedArticles);
		// check if the article of the day is already in our DB
		if (!postedArticles || !postedArticles.includes(articleOfToday)) {
			...
			await savePostedArticle(articleOfToday);
			...
			// to do: convert HTML to RichText
			...
			if (!DEBUG_MODE) await postToBluesky(articleOfToday.contentList[0].value);
			if (DEBUG_MODE) log(LogLevel.DEBUG, 'Content posted:', articleOfToday.contentList[0].value);
			...
			for (const content of articleOfToday.contentList) {
				...
				if (!alreadyPosted) {
					...
					// post the article contents to Bluesky
					// to do: convert HTML to RichText
					if (!DEBUG_MODE) await postToBluesky(content.value);
					if (DEBUG_MODE) log(LogLevel.DEBUG, 'Content posted:', content.value);
...
if (DEBUG_MODE === true) {
	...
	log(LogLevel.INFO, 'Running bot once immediately');
	runBot();
	log(LogLevel.INFO, 'Running bot once per minute...');
	...
	log(LogLevel.DEBUG, 'DEBUG_MODE is', DEBUG_MODE);
	log(LogLevel.INFO, 'Running bot every other hour...');
